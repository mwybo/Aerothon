...
################################################################################
class ACLandingskid(ACStructural):
    """
    A class for landing-gear
    
    TODO: Document attributes for the landing skid
    """
#===============================================================================
    def __init__(self):
        ...
        # Skid parameters
        self.__dict__['SkidL']        = 3 * IN   ; UnitList['SkidL']         = Length
        self.__dict__['SkidHeight']   = 1 * IN   ; UnitList['SkidHeight']    = Length
        self.__dict__['SkidWidth']    = 1.5 * IN ; UnitList['SkidWidth']     = Length

        # Internal variables
        self.__dict__['Strut'] = ACMassBox()
        self.Strut.Weight      = 1e-5 * LBF
        self.Strut.name        = 'Strut'
        self.__dict__['Skid'] = ACMassBox()
        self.Skid.Weight      = 1e-5 * LBF
        self.Skid.name        = 'Skid'
...
#===============================================================================
    def Refresh(self):
        ...
        # Position the wheel
        #
        SkidHub    = npy.array([0 * IN, self.StrutH, 0 * IN])
        Skid.X     = npy.array(Strut.X) + StrutVect + SkidHub
        Skid.Axis  = [0, 1, 0]
        Skid.LenDi = [self.SkidL, self.SkidWidth, self.SkidHeight]

        #
        # Make the wheel and strut symmetric if desired
        #
        Strut.Symmetric = self.Symmetric
        Skid.Symmetric = self.Symmetric
...
#===============================================================================
    def Draw(self, fig = 1, top = 221, side = 222, front = 223):
        ...
        self.Skid.Draw(fig, top, side, front)
...
#===============================================================================
    def _CalcStrutL(self):
        ...
        Theta = self.Theta / RAD
        return (self.GearHeight - self.SkidHeight / 2) / cos(Theta)

#===============================================================================
    def _CalcWeight(self):
        " Computes the weight of the landing gear "
        fac = 2. if self.Symmetric else 1
        return fac*(self.Strut.Weight + self.Skid.Weight)

#===============================================================================
    def SkidBase(self):
        " Computes the location where the skids rest on the ground "
        if self.dirty:
            self.Refresh()

        Skid = self.Skid

        Base = Skid.GetX() - npy.array([0,0, 0.5 * self.SkidHeight / IN])*IN
...
#===============================================================================
    def CG(self):
        ...
        CG = ((Strut.CG() * Strut.Weight + Skid.CG() * Skid.Weight) / (Strut.Weight + Skid.Weight))
...
#===============================================================================
    def CD(self, Wing, Alpha):
        ...
        SkidHeight = self.SkidHeight
        SkidWidth = self.SkidWidth
        StrutL = self.StrutL
        StrutT = self.StrutH
        ...
        STire = SkidHeight * SkidWidth
        cg = S / b      #Geometric Mean Chord
        luc = WingLower - (SkidHeight / 2)    #Distance from Tire Axle To Bottom of Wing
        Fuc = (1 - (0.04 * CLWing * cg)/luc)**2 # Thombeek pg550 G67
        
        # Thombeek pg550 G67
        Dqg1 = 1.5 * STire * Fuc
        Dqg2 = 2.7 * (DqsTire * SkidHeight * SkidWidth + DqsStrut * StrutL * StrutT)
...
#===============================================================================
    def AddToWeightTable(self, PartName, WeightTable):
        ...
        self.Skid.AddToWeightTable('Skid', WeightTable[PartName])
...
#===============================================================================
    def __getattr__(self, key):
        ...
        """
        ans = super(ACLandingskid, self).__getattr__(key)
        if ans is not None:
            return ans
